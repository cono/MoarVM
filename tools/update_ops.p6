...
sub MAIN($file = "src/core/oplist") {
    ...
    redirect-to "src/core/ops.h", {
        ...
        say '';
        say 'MVM_PUBLIC MVMOpInfo * MVM_op_get_op(unsigned short op);';
    }

    # Generate C file
    redirect-to "src/core/ops.c", {
        say '#ifdef PARROT_OPS_BUILD';
        say '#define PARROT_IN_EXTENSION';
        say '#include "parrot/parrot.h"';
        say '#include "parrot/extend.h"';
        say '#include "sixmodelobject.h"';
        say '#include "nodes_parrot.h"';
        say '#include "../../src/core/ops.h"';
        say '#else';
        say '#include "moar.h"';
        say '#endif';
        say "/* This file is generated from $file by tools/update_ops.p6. */";
